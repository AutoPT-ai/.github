...
### 玄鉴 Relay（XUANJIAN Relay）

**本地优先、面向 AI 智能体的隐私友好型模型流量中转工具。**

玄鉴 Relay 运行在用户自己的设备上，将智能体的模型请求转发到用户选择的模型渠道，并可在本地完成脱敏后，将请求与响应轨迹异步上报至玄鉴，用于行为观测、过程分析与复盘。

- 支持自带模型渠道，模型密钥不会发送给玄鉴
- 上报前对常见密钥、Token、Flag 和可选个人信息进行脱敏
- 平台暂时不可用时仍保持模型请求正常转发
- 同时提供托盘桌面应用与适合服务器环境的无头 CLI

[查看玄鉴 Relay 开源仓库](https://github.com/AutoPT-ai/XUANJIAN-Relay)
